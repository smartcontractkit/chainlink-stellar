...
// UpgradeInput upgrades the MCMS contract in place to a new Wasm hash.
type UpgradeInput struct {
	ContractID  string   `json:"contract_id"`
	NewWasmHash [32]byte `json:"new_wasm_hash"`
}

// Upgrade calls `upgrade` on MCMS (owner-gated in-place self-upgrade).
var Upgrade = cldfops.NewOperation(
	"mcms:upgrade",
	stellarops.ContractDeploymentVersion,
	"Upgrades the MCMS Soroban contract in place to a new WASM hash",

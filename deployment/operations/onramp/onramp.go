...
// UpgradeInput upgrades the OnRamp contract in place to a new Wasm hash.
type UpgradeInput struct {
	ContractID  string   `json:"contract_id"`
	NewWasmHash [32]byte `json:"new_wasm_hash"`
}

// Upgrade calls `upgrade` on OnRamp (owner-gated in-place self-upgrade).
var Upgrade = cldfops.NewOperation(
	"onramp:upgrade",
	stellarops.ContractDeploymentVersion,
	"Upgrades the OnRamp Soroban contract in place to a new WASM hash",

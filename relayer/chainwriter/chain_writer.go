...
func (s *stellarChainWriter) SubmitTransaction(ctx context.Context, contractName, method string, args any, transactionID string, toAddress string, meta *commontypes.TxMeta, value *big.Int) error {
	...
	// Stellar transactions do not support manually setting a transaction-level gas limit.
	// Resource limits (instructions, bytes) are strictly derived from the SorobanTransactionData
	// generated by the simulation, and the total fee is MinResourceFee + InclusionFee.
	if meta != nil && meta.GasLimit != nil {
		return commontypes.ErrSettingTransactionGasLimitNotSupported
	}

...
use soroban_sdk::{
    ...
    BytesN, Env, Map, Symbol, TryFromVal, TryIntoVal, Val, Vec,
};
...
/// After a bps fee accrues on the siloed pool's own balance during `lock_or_burn`
/// (the full amount is pulled to the pool; the post-fee `dest` is escrowed in the
/// siloed lockbox, leaving only the fee on the pool), the owner sweeps the full
/// accrued balance to a recipient via `withdraw_fee_tokens`. The recipient receives
/// the fee and the pool balance drops to 0. Gate is `require_owner_or_fee_admin`
/// (EVM `TokenPool.onlyOwnerOrFeeAdmin`). Closes the siloed-side coverage gap where
...
fn test_withdraw_fee_tokens_sweeps_accrued() {
    ...
    // fee = 1000e18 * 100 / 10000 = 10e18; dest (990e18) is in the lockbox.
    let fee: i128 = 10 * SILOED_E18;
    let pool_addr = t.pool_client.address.clone();
    assert_eq!(t.tc.balance(&pool_addr), fee);
    ...
    let recipient = Address::generate(&t.env);
    t.pool_client.withdraw_fee_tokens(
        &t.owner,
        &Vec::from_array(&t.env, [t.token_addr.clone()]),
        &recipient,
    );
    assert_eq!(t.tc.balance(&recipient), fee);
    assert_eq!(t.tc.balance(&pool_addr), 0);
}
...
// SiloedLockReleaseTokenPool has no `peek`, so a successful `peek` at the pool
// address after `upgrade` proves the executable was swapped in place. No
// `stellar` CLI in this env, so we reuse this fixture instead of adding one.
...
fn setup_pool_for_upgrade(env: &Env) -> SiloedLockReleaseTokenPoolContractClient<'static> {
    let contract_id = env.register(SiloedLockReleaseTokenPoolContract, ());
    let client = SiloedLockReleaseTokenPoolContractClient::new(env, &contract_id);
    let owner = Address::generate(env);
    client.initialize(
        &owner,
...
fn test_upgrade_by_owner_swaps_executable_and_emits_event() {
    let env = Env::default();
    env.mock_all_auths();
    let client = setup_pool_for_upgrade(&env);
    ...
    // SiloedLockReleaseTokenPool has no `peek`; the fixture does. A successful
    // `peek` at the pool address proves the executable was swapped in place.
    // Instance storage is preserved by `update_current_contract_wasm` (host
    // guarantee), and the pool never wrote the fixture's "slot" key, so peek
    // reads back 0.
    ...
    assert_eq!(
        peeked, 0,
        "fixture peek must run at the siloed lock-release pool address after upgrade"
    );
}

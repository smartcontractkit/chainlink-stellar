...
use soroban_sdk::{
    symbol_short, testutils::Address as _, testutils::Events as _, testutils::Ledger, token, vec,
...
// LockReleaseTokenPool has no `peek`, so a successful `peek` at the pool
...
fn test_upgrade_by_owner_swaps_executable_and_emits_event() {
    ...
    // LockReleaseTokenPool has no `peek`; the fixture does. A successful `peek`
    // at the pool address proves the executable was swapped in place. Instance
    ...
    assert_eq!(
        peeked, 0,
        "fixture peek must run at the lock-release pool address after upgrade"
    );
}

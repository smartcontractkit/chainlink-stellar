...
use data_feeds_common::{TokenRecoverable, Upgradeable, Versioned, WasmHash};
...
use crate::events::{
    FeedAdminAdded, FeedAdminRemoved, FeedConfigRemoved, FeedConfigSet, FeedUpdated,
    InvalidUpdatePermission, StaleReport,
};
use crate::interface::types::{RoundData, WorkflowName, WorkflowOwner, WorkflowPermission};
...
impl DataFeedsCacheReader for DataFeedsCache {
    fn latest_round(env: Env, data_id: DataId) -> Result<Option<RoundData>, CacheError> {
        Ok(feed::feed_state(&env, &data_id).map(|t| t.latest_round))
    }
    ...
    fn decimals(_env: Env, data_id: DataId) -> Result<u32, CacheError> {
        Ok(feed::decimals(&data_id))
    }

    fn description(env: Env, data_id: DataId) -> Result<String, CacheError> {
...
impl DataFeedsCacheAdmin for DataFeedsCache {
    fn set_feed_configs(
        ...
        for entry in entries.iter() {
            ...
            FeedConfigSet {
                data_id: data_id.clone(),
                decimals: feed::decimals(&data_id),

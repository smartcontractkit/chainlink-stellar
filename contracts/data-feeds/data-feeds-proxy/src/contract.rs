...
use data_feeds_cache::{DataFeedsCacheReaderClient, DataId};
...
impl DataFeedsProxyReader for DataFeedsProxy {
    ...
    fn decimals(env: Env, data_id: DataId) -> Result<u32, ProxyReadError> {
        storage::extend_ttl(&env);
        Ok(DataFeedsCacheReaderClient::new(&env, &storage::get_cache(&env)).decimals(&data_id))
    }

    fn description(env: Env, data_id: DataId) -> Result<String, ProxyReadError> {
        storage::extend_ttl(&env);
        Ok(DataFeedsCacheReaderClient::new(&env, &storage::get_cache(&env)).description(&data_id))
    }
}

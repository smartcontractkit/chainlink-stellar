...
// QuorumConfigKey is a Soroban discriminated-union (#[contracttype] enum with payload(s)).
...
type QuorumConfigKey struct {
	SourceChainSelector *QuorumConfigKeySourceChainSelector
}

// QuorumConfigKeySourceChainSelector is the tuple variant QuorumConfigKey::SourceChainSelector.
type QuorumConfigKeySourceChainSelector struct {
	Field0 uint64
}

// ToScVal converts QuorumConfigKey to its Soroban discriminated-union encoding.
// Returns an error if zero or multiple variant pointers are set.
func (e QuorumConfigKey) ToScVal() (xdr.ScVal, error) {
	set := 0
	if e.SourceChainSelector != nil {
		set++
	}
	if set != 1 {
		return xdr.ScVal{}, fmt.Errorf("QuorumConfigKey: expected exactly one variant set, got %d", set)
	}
	if e.SourceChainSelector != nil {
		items := []xdr.ScVal{
			scval.SymbolToScVal("SourceChainSelector"),
			scval.Uint64ToScVal(e.SourceChainSelector.Field0),
		}
		return scval.VecToScVal(items), nil
	}
	return xdr.ScVal{}, fmt.Errorf("QuorumConfigKey: unreachable")
}

// QuorumConfigKeyFromScVal parses an xdr.ScVal into QuorumConfigKey.
func QuorumConfigKeyFromScVal(val xdr.ScVal) (QuorumConfigKey, error) {
	vecPtr, ok := val.GetVec()
	if !ok || vecPtr == nil || *vecPtr == nil {
		return QuorumConfigKey{}, fmt.Errorf("expected vec for QuorumConfigKey enum")
	}
	vec := *vecPtr
	if len(vec) < 1 {
		return QuorumConfigKey{}, fmt.Errorf("QuorumConfigKey: empty vec")
	}
	tag, err := scval.SymbolFromScVal(vec[0])
	if err != nil {
		return QuorumConfigKey{}, fmt.Errorf("QuorumConfigKey: variant tag: %w", err)
	}
	switch tag {
	case "SourceChainSelector":
		if len(vec) != 2 {
			return QuorumConfigKey{}, fmt.Errorf("QuorumConfigKey::SourceChainSelector: expected 2 elements, got %d", len(vec))
		}
		payload := &QuorumConfigKeySourceChainSelector{}
		if v, err := scval.Uint64FromScVal(vec[1]); err != nil {
			return QuorumConfigKey{}, fmt.Errorf("QuorumConfigKey::SourceChainSelector[0]: %w", err)
		} else {
			payload.Field0 = v
		}
		return QuorumConfigKey{SourceChainSelector: payload}, nil
	default:
		return QuorumConfigKey{}, fmt.Errorf("QuorumConfigKey: unknown variant %q", tag)
	}
}

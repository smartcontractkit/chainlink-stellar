...
func (c *BurnMintPoolClient) Initialize(ctx context.Context, owner string, token string, tokenDecimals uint32, router string, rampRegistry string, rmnProxy string) error {
	args := []xdr.ScVal{
		scval.AddressToScVal(owner),
...
func (c *BurnMintPoolClient) AddRemotePool(ctx context.Context, remoteChainSelector uint64, remotePoolAddress []byte) error {
	args := []xdr.ScVal{
...
func (c *BurnMintPoolClient) GetRemotePools(ctx context.Context, remoteChainSelector uint64) ([][]byte, error) {
	args := []xdr.ScVal{
		scval.Uint64ToScVal(remoteChainSelector),
	}
...
func (c *BurnMintPoolClient) RemoveRemotePool(ctx context.Context, remoteChainSelector uint64, remotePoolAddress []byte) error {
	args := []xdr.ScVal{
		scval.Uint64ToScVal(remoteChainSelector),
		scval.BytesToScVal(remotePoolAddress),
	}
...
func (c *BurnMintPoolClient) WaitForRemotePoolAddedEvent(ctx context.Context, startLedger uint32, timeout time.Duration, filter func(*RemotePoolAddedEvent) bool) (*RemotePoolAddedEvent, error) {
	startTime := time.Now()
	ticker := time.NewTicker(2 * time.Second)
	defer ticker.Stop()
...
func (c *BurnMintPoolClient) WaitForRemotePoolRemovedEvent(ctx context.Context, startLedger uint32, timeout time.Duration, filter func(*RemotePoolRemovedEvent) bool) (*RemotePoolRemovedEvent, error) {
	startTime := time.Now()
	ticker := time.NewTicker(2 * time.Second)
	defer ticker.Stop()

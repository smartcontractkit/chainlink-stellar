...
// GenerateClient renders the contract client in one of two styles.
//
// style "direct": each method executes immediately; whether it simulates or
// submits a transaction is decided at generation time — readOnly, when
// non-nil, is the authoritative set of read-only functions, and when nil the
// legacy name heuristic in isReadOnlyFunction applies.
//
// style "twostep": each method builds and returns a *bindings.Call without
// executing anything; the caller chooses Result (free simulation) or
// SignAndSend (fee-paying transaction) per call site. No read-only
// classification exists in this style (readOnly must be nil).
func GenerateClient(pkg string, contract *Contract, readOnly map[string]bool, style string) string {
	knownEnumNames = map[string]bool{}
	for _, e := range contract.Enums {
		knownEnumNames[e.Name] = e.IsIntEnum()
	}

	// Keep alias resolution in sync with GenerateTypes: client generation
	// also resolves aliases (event parsing, mappableType), so it must not
	// depend on GenerateTypes having run first.
	knownAliases = map[string]string{}
	for _, a := range contract.Aliases {
		knownAliases[a.Name] = a.Target
	}
	...
	// Imports. In twostep style the methods themselves take no context (it is
	// passed at execution time, to Call.Result/SignAndSend), so context is
	// only needed for the event helpers.
	b.WriteString("import (\n")
	if style != "twostep" || len(contract.Events) > 0 {
	...
	for _, fn := range contract.Functions {
		...
		if style == "twostep" {
			generateMethodTwoStep(&b, contract, fn)
		} else {
...
// generateMethodTwoStep renders a method that builds and returns a
// *bindings.Call[T] instead of executing. The decode closure reuses the same
// return-value parsing as the direct style; gofmt (formatOrDie) normalizes
// the closure body's indentation.
func generateMethodTwoStep(b *strings.Builder, contract *Contract, fn Function) {
	methodName := snakeToPascal(fn.Name)
	returnType, returnsValue := parseReturnType(fn.Returns)

	if returnsValue && isTupleType(returnType) {
		fmt.Fprintf(os.Stderr, "%s: %s returns a tuple, which the twostep style does not support (add a named struct or generator support)\n", contract.Name, fn.Name)
		os.Exit(1)
	}

	// T for bindings.Call[T]: same pointer rules as the direct style's
	// return type, with Void standing in for functions that return nothing.
	goT := "bindings.Void"
	if returnsValue {
		goT = rustTypeToGo(returnType)
		if !strings.HasPrefix(returnType, "Option<") && isStructType(returnType) {
			goT = "*" + goT
		}
	}
	...
	var params []string
	for _, input := range fn.Inputs {
		params = append(params, fmt.Sprintf("%s %s", snakeToCamel(input.Name), rustTypeToGo(input.Type)))
	}

	b.WriteString(fmt.Sprintf("// %s builds a call to the %s contract function.\n", methodName, fn.Name))
	b.WriteString("// Execute it with Result (free simulation, state changes discarded) or\n")
	b.WriteString("// SignAndSend (fee-paying transaction).\n")
	b.WriteString(fmt.Sprintf("func (c *%sClient) %s(%s) *bindings.Call[%s] {\n", contract.Name, methodName, strings.Join(params, ", "), goT))

	if len(fn.Inputs) > 0 {
		b.WriteString("\targs := []xdr.ScVal{\n")
		for _, input := range fn.Inputs {
			b.WriteString(fmt.Sprintf("\t\t%s,\n", getArgConverter(input.Type, snakeToCamel(input.Name))))
		}
		b.WriteString("\t}\n")
	} else {
		b.WriteString("\targs := []xdr.ScVal{}\n")
	...
	b.WriteString(fmt.Sprintf("\treturn bindings.NewCall(c.invoker, c.contractID, %q, args, func(result *xdr.ScVal) (%s, error) {\n", fn.Name, goT))
	if returnsValue {
		b.WriteString("\tif result == nil {\n")
		b.WriteString(fmt.Sprintf("\t\treturn %s, fmt.Errorf(\"no return value from %s\")\n", zeroValue(returnType), fn.Name))
		b.WriteString("\t}\n")
		...
		b.WriteString("\t_ = result // void return\n")
		b.WriteString("\treturn bindings.Void{}, nil\n")
	}
	b.WriteString("\t})\n")
	b.WriteString("}\n\n")
}
...
func mappableType(rustType string) (string, bool) {
	...
	case strings.HasPrefix(t, "soroban_sdk::Vec<"):
		return mappableType(extractVecInnerType(t))

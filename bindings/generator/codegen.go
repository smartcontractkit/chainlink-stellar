...
// input unchanged.
func resolveAlias(rustType string) string {
	if target, ok := knownAliases[extractStructName(rustType)]; ok {
		return target
	}
	return rustType
}
...
func typesFileNeedsImports(contract *Contract) (needFmt, needScval, needXdr, needBig bool) {
	...
	return needFmt, needScval, needXdr, needBig
}
...
func GenerateTypes(pkg string, contract *Contract) string {
	...
	needFmt, needScval, needXdr, needBig := typesFileNeedsImports(contract)
	if needFmt || needScval || needXdr || needBig {
...
// generateAlias emits a defined Go type for a `pub type X = Target;` alias.
// Only BytesN<N> targets get a defined type + converters (matching the
// struct converter conventions so default encode/decode cases work
// unchanged); I256 aliases resolve inline to *big.Int via
// resolveAlias/rustTypeToGo and emit no defined type here. U256 has no
// mapping anywhere else in the pipeline (rustTypeToGo, getToScValConverter,
// generateFromScValField all lack a case for it), so a U256 alias must panic
// rather than silently emit broken/incomplete output — mirror the I256
// wiring throughout the pipeline before allowing it here.
func generateAlias(b *strings.Builder, a TypeAlias) {
	if n := extractBytesNSize(a.Target); n > 0 {
		b.WriteString(fmt.Sprintf("// %s is an alias for BytesN<%d> in the contract interface.\n", a.Name, n))
		b.WriteString(fmt.Sprintf("type %s [%d]byte\n\n", a.Name, n))
		b.WriteString(fmt.Sprintf("func (v %s) ToScVal() (xdr.ScVal, error) {\n\treturn scval.Bytes%dToScVal([%d]byte(v)), nil\n}\n\n", a.Name, n, n))
		b.WriteString(fmt.Sprintf("func %sFromScVal(val xdr.ScVal) (*%s, error) {\n", a.Name, a.Name))
		b.WriteString(fmt.Sprintf("\tb, err := scval.Bytes%dFromScVal(val)\n\tif err != nil {\n\t\treturn nil, err\n\t}\n", n))
		b.WriteString(fmt.Sprintf("\tv := %s(b)\n\treturn &v, nil\n}\n\n", a.Name))
		return
	}
	if a.Target == "soroban_sdk::I256" {
		return
	}
	if a.Target == "soroban_sdk::U256" {
		panic(fmt.Sprintf("unsupported type alias target %q for %s — U256 alias support is not implemented (no mapping in rustTypeToGo/getToScValConverter/generateFromScValField); extend generateAlias and the rest of the pipeline together", a.Target, a.Name))
	}
	panic(fmt.Sprintf("unsupported type alias target %q for %s — extend generateAlias", a.Target, a.Name))
}
...
// encodings. The decision is IsIntEnum, not IsUnit: only enums where every
// variant is unit-shaped AND at least one variant has an explicit `= N`
// discriminant become Go uint32 newtypes (ScVal::U32 wire format). All other
// enums — including unit-only "symbolic unions" with no discriminant
// anywhere (e.g. `enum Bound { AtOrBefore, AtOrAfter }`), plus any enum with
// a tuple/struct variant — become a discriminated union encoded as
